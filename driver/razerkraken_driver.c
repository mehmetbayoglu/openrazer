...
/* The 0xe0 "meta" write. Byte 3 is the "activate this slot now" flag: with it
 * clear, a preset switch stores the slot's band data but the firmware never
 * makes it active, so a GUI/sysfs write completes cleanly (urb_status=0) yet
 * the headset's real EQ never changes and only the on-board button appears to
 * work. It must be 0x01 on every preset. Confirmed three ways: a fresh Windows
 * Synapse preset-switch capture (ShenaniganJoe, 2026-08-07) shows 0x01 here on
 * all six switches with no exceptions; the BlackShark V3 Pro for Xbox capture
 * on #2794 carries it too; and enumerating cls=0x60 per slot on a live V3
 * dongle put the flag on the active slot alone and moved it as the preset
 * changed. */
static const u8 bs_v3pro_eq_meta[5][7] = {
    { 0x00, 0x00, 0x01, 0x01, 0x00, 0x00, 0x00 },
    { 0x01, 0x01, 0x01, 0x01, 0x01, 0x00, 0x00 },
    { 0x02, 0x03, 0x01, 0x01, 0x03, 0x00, 0x00 },
    { 0x03, 0x02, 0x01, 0x01, 0x02, 0x00, 0x00 },
    { 0x04, 0x04, 0x01, 0x01, 0x0b, 0x00, 0x00 },
};
...
static const u8 bs_v3pro_eq_commit[5][12] = {
    ...
    /* Esports: the three flag bytes sit one position later than the other
     * rows (idx 4,5,6 not 3,4,5), with idx 3 clear. The old row had them at
     * 3,4,5, which is a misaligned commit frame for this slot. Corrected to
     * ShenaniganJoe's checksum-verified Synapse capture (2026-08-07). */
    { 0x04, 0x00, 0x00, 0x00, 0x01, 0x01, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00 },
};
